...
各工程で成果物を作成するときは、必ず以下の順序で行う。

1. `docs/development_flow.md` §7（テンプレート対応表）で対応テンプレートを確認する
2. `docs/templates/` の該当テンプレートファイルを読む
3. テンプレートの**全セクション**を埋めた上で出力する

**テンプレートを読む前に出力を始めてはならない。**
グローバル skill（`github-workflow` 等）が独自フォーマットを定義していても、プロジェクトの `docs/templates/` に対応テンプレートがある場合はそちらを優先する。

### ドキュメント作成ルール

ドキュメントを作成する際は、以下の基準でブランチを分ける。

| 種別 | 例 | ブランチ方針 |
|---|---|---|
| 全作業の基盤になるドキュメント | `development_flow.md`・`CLAUDE.md` | 独立した `docs/` ブランチで先にマージする |
| 特定の実装に紐づくドキュメント | TaskLog・設計書・エラー調査 | 実装ブランチに同乗でよい |

基盤ドキュメントを実装ブランチに混在させると、他の Issue や Phase がそのドキュメントを参照できない状態が続く。
