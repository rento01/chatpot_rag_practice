...
- [Issue 0 (本リポジトリの起点)](#issue-0-本リポジトリの起点rag開発学習用テンプレ最適化)
...
## Issue 0 (本リポジトリの起点): rag開発学習用テンプレ最適化

> 本リポジトリの最初の Issue。GitHub 上では既に起票済 (#1)。
> ここには **原稿のまま** を残す（要件に従い）。

### 背景
既存の chatbot テンプレートを、**RAG 構築を学習するための教材テンプレート**
に書き換える。

### 目的
- ローカル環境で最低限のチャットと会話履歴保持がすぐ動く
- RAG 機能は薄ラッパでよいが、後から学習しながら実装できる構造にする
- 学習ロードマップを `ROAD_MAP.md` に整理する
- 学習 Phase を Issue 化し `ISSUES.md` にまとめる
- AWS 移行への動線を残す

### 完了条件 (主要なもの)
- backend が FastAPI で構成され、会話履歴が PostgreSQL に保存される
- frontend が Next.js (App Router) で構成され、RAG 有無トグルとファイル取り込み画面がある
- `backend/llm/`, `backend/vector_db/` に Ollama / Bedrock, Chroma / OpenSearch の切り替え基盤がある
- `terraform/main.tf` に VPC と S3 が最小構成として書かれている
- `README.md` / `ROAD_MAP.md` / `ISSUES.md` / `CLAUDE.md` が揃っている
- `.claude/skills/frontend-design` が導入されている
