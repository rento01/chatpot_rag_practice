/**
 * バックエンド (FastAPI) との通信を 1 ファイルに集約する。
 *
 * 教材として、初学者が「どの API が叩かれているか」を
 * ここを見るだけで把握できることを優先している。
 */
...
async function request(path: string, options: RequestOptions = {}): Promise<Response> {
  ...
  // タイムアウト + 外部 abort を AbortController に集約する
...
// ──────────────────────────────────────────────
// コレクション (Collection) / ドキュメント (Document)
// ──────────────────────────────────────────────
...
export async function* streamChat(
  ...
  const { conversationId, collectionId, useRag, signal } = options;
